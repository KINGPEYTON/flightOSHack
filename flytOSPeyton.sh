...
apt-key adv --keyserver 'hkp://keyserver.ubuntu.com:80' --recv-key C1CF6E31E6BADE8868B172B4F42ED6FBAB17C654 || ( echo -e "\033[0;31m Are You Connected to the Internet? \n Exiting \033[0m" ; exit 1 )
...
elif [ $DEVICE == "Edison" ]; then   

    wget -O $HOME/flytos.deb $(curl -sS https://my.flytbase.com/api/downloads/latest/Intel%20Edison/ | python -c "import sys, json; print json.load(sys.stdin)['link']") || (echo -e "\033[0;31m Unable to get flytos debian package. Are you connected to the internet? \033[0m" ; exit 1)

    # Check md5sum of downloaded file, continue if match, retry if fail, quit on another fail.
    if [ $(md5sum $HOME/flytos.deb | awk -F ' ' '{print $1}') = $(curl -sS https://my.flytbase.com/api/downloads/latest/Intel%20Edison/ | python -c "import sys, json; print json.load(sys.stdin)['md5hash']") ]; then
        echo "md5sum match, continuing with installation"
    else
        echo -e "\033[0;31m md5sum match: FAIL \033[0m"
        echo "Redownloading FlytOS package"
        rm $HOME/flytos.deb
        wget -O $HOME/flytos.deb $(curl -sS https://my.flytbase.com/api/downloads/latest/Intel%20Edison/ | python -c "import sys, json; print json.load(sys.stdin)['link']")
        if [ $(md5sum $HOME/flytos.deb | awk -F ' ' '{print $1}') = $(curl -sS https://my.flytbase.com/api/downloads/latest/Intel%20Edison/ | python -c "import sys, json; print json.load(sys.stdin)['md5hash']") ]; then
            echo "MD5sum match, continuing"
        else
            echo -e "\033[0;31m Problem downloading FlytOS debian package, please check internet connection and try again. \033[0m"
